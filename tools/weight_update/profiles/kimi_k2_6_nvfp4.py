"""Profile a verified Kimi K2.6 NVFP4 delta lineage on Modal.

The entrypoint downloads NVIDIA's pinned serving checkpoint, builds a
standardized element-wise synthetic delta lineage, and verifies repeated and
folded updates.

    MODAL_FUNCTION_RUNTIME=runc uv run --extra modal modal run -d \
      tools/weight_update/profiles/kimi_k2_6_nvfp4.py \
