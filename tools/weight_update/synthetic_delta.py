...
def append_reversal_delta(
    source_dir: str,
    *,
    reverse_version: int,
    commit: Callable[[], None],
) -> dict[str, Any]:
    """Append an XOR delta that reverses one existing lineage step.

    Applying an XOR payload twice restores the prior bytes. Reusing payloads
    whose tensors also changed in the preceding version therefore gives the
    fault-injection benchmark a valid, large next update without regenerating
    another full synthetic lineage. Other tensors retain their current bytes.
    """

    import safetensors
    import safetensors.numpy

    if reverse_version < 2:
        raise ValueError("reverse_version must have a preceding target")

    root = Path(source_dir)
    source_version = reverse_version
    prior_version = reverse_version - 1
    target_version = reverse_version + 1
    source_dir_path = root / f"weight_v{source_version:06d}"
    prior_dir_path = root / f"weight_v{prior_version:06d}"
    target_dir_path = root / f"weight_v{target_version:06d}"

    source_index_path = source_dir_path / "model.safetensors.index.json"
    prior_index_path = prior_dir_path / "model.safetensors.index.json"
    for index_path in (source_index_path, prior_index_path):
        if not index_path.is_file():
            raise FileNotFoundError(f"delta index is missing: {index_path}")
    ...
    prior_metadata = prior_index.get("metadata") or {}
    expected_source_metadata = {
        "version": f"{source_version:06d}",
        "base_version": f"{prior_version:06d}",
        "delta_encoding": "xor",
    }
    expected_prior_metadata = {
        "version": f"{prior_version:06d}",
        "delta_encoding": "xor",
    }
    for metadata, expected, index_path in (
        (source_metadata, expected_source_metadata, source_index_path),
        (prior_metadata, expected_prior_metadata, prior_index_path),
    ):
        if any(metadata.get(key) != value for key, value in expected.items()):
            raise ValueError(f"delta lineage metadata is invalid: {index_path}")
    source_weight_map = source_index.get("weight_map") or {}
    prior_weight_map = prior_index.get("weight_map") or {}
    reversed_names = set(source_weight_map) & set(prior_weight_map)
    if not reversed_names:
        raise ValueError("adjacent deltas have no tensors in common")
    target_weight_map = {
        name: filename
        for name, filename in source_weight_map.items()
        if name in reversed_names
    }

    expected_metadata = {
        "version": f"{target_version:06d}",
        "base_version": f"{source_version:06d}",
        "delta_encoding": "xor",
        "synthetic_reversal_of_version": f"{source_version:06d}",
    }
    target_index_path = target_dir_path / "model.safetensors.index.json"
    if target_index_path.is_file():
        target_metadata = (
            json.loads(target_index_path.read_text()).get("metadata") or {}
        )
        if all(
            target_metadata.get(key) == value
            for key, value in expected_metadata.items()
        ):
            result = {
                "version": target_version,
                "reverses_version": source_version,
                "reused": True,
            }
            print(f"Reusing reversal delta at {target_dir_path}")
            print(f"SYNTHETIC_REVERSAL_DELTA={json.dumps(result, sort_keys=True)}")
            return result
        raise ValueError(f"existing reversal delta is incompatible: {target_dir_path}")

    prior_checksums: dict[str, str] = {}
    for filename in sorted(set(prior_weight_map.values())):
        names = {name for name, shard in prior_weight_map.items() if shard == filename}
        with safetensors.safe_open(
            prior_dir_path / filename,
            framework="np",
        ) as handle:
            metadata = handle.metadata() or {}
        if set(metadata) != names:
            raise ValueError(
                f"delta checksum/index tensor mismatch for {prior_dir_path / filename}"
            )
        prior_checksums.update(metadata)

    temporary_dir = Path(tempfile.mkdtemp(prefix=f".{target_dir_path.name}-", dir=root))
    try:
        wire_bytes = 0
        for filename in sorted(set(source_weight_map.values())):
            tensors = {
                name: tensor
                for name, tensor in safetensors.numpy.load_file(
            ...
            if not tensors:
                continue
            metadata = {name: prior_checksums[name] for name in tensors}
            output_path = temporary_dir / filename
            safetensors.numpy.save_file(tensors, output_path, metadata=metadata)
            wire_bytes += output_path.stat().st_size

        metadata = dict(source_metadata)
        metadata.update(expected_metadata)
        (temporary_dir / "model.safetensors.index.json").write_text(
            json.dumps({"metadata": metadata, "weight_map": target_weight_map})
        )
        os.replace(temporary_dir, target_dir_path)
    finally:
    ...
    result = {
        "version": target_version,
        "reverses_version": source_version,
        "tensors": len(target_weight_map),
        "source_tensors": len(source_weight_map),
        "omitted_tensors": len(source_weight_map) - len(target_weight_map),
        "wire_bytes": wire_bytes,
        "reused": False,
    }
    print(f"Committed reversal delta at {target_dir_path}")
    print(f"SYNTHETIC_REVERSAL_DELTA={json.dumps(result, sort_keys=True)}")
    return result
