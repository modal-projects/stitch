...
from stitch.pools.modal_flash import ModalFlashPool, _host, _normalize_url, _replica_urls


def test_replica_request_routes_through_the_pool_url() -> None:
    # Direct container URLs sit behind relay auth; per-replica requests ride the
    # pool's own Flash URL pinned by modal-flash-upstream.
    pool = ModalFlashPool("app", "Server")
    pool._upstream_url_cache = "https://pool.modal.run"

    url, headers = pool.replica_request("https://h-ta-123.w.modal.host/", "/server_info")
