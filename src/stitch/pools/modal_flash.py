...
class ModalFlashPool(Pool):
    ...
    def _upstream_url(self) -> str:
        """The upstream class's own Flash URL (not an LB override), cached for the
        pool client's lifetime — it is stable for a deployed app."""
    ...
    def replica_request(self, replica: str, path: str) -> tuple[str, dict[str, str]]:
        # Direct container URLs sit behind relay auth; a specific replica is
        # reached through the pool's own Flash URL pinned by modal-flash-upstream —
        # the same edge path traffic takes (cookbook/common/router.py's registry
        # polls work the same way). The edge keys upstreams by host:port, and
        # discover_replicas' https URLs imply 443.
