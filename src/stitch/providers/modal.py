...
def volume_committer(volume_name: str) -> Callable[[], None]:
    return lambda: commit_volume(volume_name)
...
async def resolve_flash_gateway_url_aio(app_name: str, cls_name: str) -> str:
    import modal

    cls = modal.Cls.from_name(app_name, cls_name)
    urls = await cls._experimental_get_flash_urls.aio()
    if not urls:
        raise RuntimeError(
            f"No Flash gateway URL found for {app_name}.{cls_name}. "
            "Deploy the app first so Modal starts the Flash pool."
...
def wake_targets(targets: list[str], version: int, *, timeout: float = 5.0) -> None:
    ...
    def wake_one(target: str) -> None:
        ...
        try:
            resp = requests.post(url, json={"target_version": int(version)}, timeout=timeout)
...
async def wake_targets_aio(targets: list[str], version: int, *, timeout: float = 5.0) -> None:
    await asyncio.to_thread(wake_targets, targets, version, timeout=timeout)
