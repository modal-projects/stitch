...
the trainer. A resume keeps that identity and its deployed rollout pool; changing
the experiment or forking a checkpoint is a new launch with a new run id.
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--resume-from",
        metavar="RUN_ID",
        help="resume RUN_ID from its latest saved Megatron/HF checkpoint",
    )
    parser.add_argument(
        "--auto-resume",
        action="store_true",
...
def main() -> None:
    ...
    if args.resume_from is not None:
        ...
        resume_point = None
    run_id = args.resume_from or uuid.uuid4().hex[:8]
    _set_attempt_env(os.environ, run_id=run_id, resume_point=resume_point)
    _run_attempt(deploy=resume_point is None, supervise=False)
...
def _run_auto_resume(exp: Any, resume_from: str | None) -> None:
    ...
    deploy = resume_point is None
    while True:
        _set_attempt_env(os.environ, run_id=run_id, resume_point=resume_point)
        try:
            _run_attempt(deploy=deploy, supervise=True)
            return
        ...
        print(
            "Trainer stopped unexpectedly; resuming "
            f"rollout {resume_point.rollout_id} at weight v"
            f"{resume_point.weight_version} in run {run_id}",
            flush=True,
        )
...
def _resume_point_for_run(exp: Any, source_run_id: str) -> ResumePoint:
    ...
    print(
        ...
        f"rollout_id={resume_point.rollout_id}, "
...
def _resume_point_after_failure(
    ...
    try:
        ...
        print(
            ...
            f"retrying rollout {previous.rollout_id} at weight "
            f"v{previous.weight_version} from run "
...
def _set_attempt_env(
    ...
    *,
    ...
    print(
        f"Resuming run {run_id} from rollout {resume_point.rollout_id}; "
        f"restoring Stitch weight_v{resume_point.weight_version:06d}",
        flush=True,
    )


def _run_attempt(*, deploy: bool, supervise: bool) -> None:
