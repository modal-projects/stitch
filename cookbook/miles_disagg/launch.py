...
A fresh launch mints a run id, deploys the run's pool, waits for its floor,
and spawns the trainer. Recovery from a trainer preemption or crash is Modal's
job, not this script's: the trainer retries with identical input and
re-derives its resume state from the run volume (see
``cookbook.miles_disagg.resume``), so nothing here supervises a running
trainer and a live pool is never redeployed.

``--resume-from RUN_ID`` reuses an existing run's identity — for a run past
its retry budget or a manual takeover. It cancels the run's recorded trainer
call, then spawns a successor on the deployed pool, which resumes like any
retry; the pool itself is never touched here. If the pool is gone (or its
infra must change first), deploy it explicitly under the same run id:

    EXPERIMENT_CONFIG=<experiment> RUN_ID=<run> uv run --extra modal modal deploy -m cookbook.miles_disagg.app

Minting the id here, before importing the pool module, lets the pool's app
name resolve from ``RUN_ID`` at import. A plain script, not a ``modal run``
entrypoint: ``App.deploy()`` only persists outside a ``modal run`` session.
"""
...
def _cancel_recorded_trainer_call(run: Any) -> None:
    """Cancel the run's live trainer call and wait for it to stop, so the
    takeover never briefly runs two trainers against one run."""
    import modal
    ...
    # A cancelled call never yields an output through .get() — its inputs settle
    # (as FAILURE) only in the call graph.
