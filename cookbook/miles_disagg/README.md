# Disaggregated miles on Modal (NVFP4)

Train Kimi K2.6 (or Moonlight) with GRPO under **native NVFP4 QAT** while
rollouts run on a separate, elastic pool of SGLang servers. The miles twin of
[`../slime_disagg`](../slime_disagg): same two-half architecture and the same
`stitch` bulletin-board / sidecar machinery, but the trainer is **miles** and
the precision is **NVFP4**.

The app has two halves.

- **`Trainer`** is a clustered miles/Ray job. It QAT-trains in NVFP4
  (`--fp4-format e2m1`, `NVFP4BlockScaling`), and after each step writes a sparse
  XOR weight delta to a Modal Volume (the "bulletin board") and publishes a new
  weight version.
- **`Server`** is a Modal Flash pool of SGLang servers on Blackwell. A sidecar
  in each container watches the bulletin board, applies deltas in order, and
  serves rollouts pinned to an exact weight version. Requests for a version a
  container hasn't reached get `409` and miles retries.
