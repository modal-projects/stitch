...
@app.cls(
    ...
    # Modal owns the retry loop: a failed attempt reruns with identical input
    # and train() re-derives its resume state from the run volume. Single-use
    # containers make every attempt a complete fresh gang — enter() reruns and
    # Ray bootstraps clean on every rank (tools/probes/gang_lifecycle_spike.py);
    # workers hold their containers for the attempt's lifetime in train().
...
class Trainer:
    """miles actor cluster. Containers are single-use, so every attempt brings Ray
    up fresh in enter()."""
    ...
    def train(self, payload: dict) -> None:
        ...
        run instead of replaying the original attempt's world. (Resume state
        needs the Modal Volume store; on other backends a retry past the first
        publish fails its claim's rewind guard rather than relabel history.)"""
        ...
        if self.rank != 0:
            # Hold this worker's container for the attempt: rank 0 alone decides
            # the call's outcome, and returning now would exit the single-use
            # container under its Ray node.
...
def spawn_train() -> Any:
    ...
    lets a takeover cancel the live call before spawning its successor."""
