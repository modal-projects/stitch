...
def _target_version_of(info: dict[str, Any]) -> int | None:
    """``target_version``, else ``metrics.target_version`` (older sidecars)."""
    target = info.get("target_version")
    if target is None:
        metrics = info.get("metrics")
        if isinstance(metrics, dict):
            target = metrics.get("target_version")
    return target
...
def fleet_is_mixed(infos: list[dict[str, Any]]) -> tuple[bool, dict[str, Any]]:
    ...
    for info in infos:
        ...
        if sync_state.lower() not in _FLEET_TRANSITION_STATES:
            continue
        target = _target_version_of(info)
        if target is None:
            continue
