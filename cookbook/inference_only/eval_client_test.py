...
def test_eval_timeouts_and_cli(
    ...
    assert "Stitch-Session-Id" not in headers
    ...
    constructed = _cli_construct(monkeypatch, ["--straggler-taper"])
    assert constructed["straggler_taper"] is True
...
def test_eval_records_and_taper(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """Fleet snapshots, drain events, boundary rollout, and straggler taper."""
    harness = _Harness()
    _patch_http(monkeypatch, harness)
    client = _make_client(
        tmp_path,
    ...
    assert _make_client(tmp_path).straggler_taper is False
    client = _make_client(
        tmp_path, num_sessions=4, num_stragglers=2, straggler_taper=True
    )
    ...
    async def fake_eval_session(
        ...
        session_id: str,
        pinned_version: int,
        main_phase_deadline: float,
        straggler_deadline: float,
    ) -> None:
        recorded.append((main_phase_deadline, straggler_deadline))

    monkeypatch.setattr(ec.EvalClient, "eval_session", fake_eval_session)

    async def start() -> None:
        main_tasks, straggler_tasks, _ = client._start_sessions(1, 1)
        assert len(straggler_tasks) == 2
        await asyncio.gather(*main_tasks, *straggler_tasks)

    asyncio.run(start())
    assert len(recorded) == 4
    main_deadline = recorded[0][0]
    assert recorded[0][1] == pytest.approx(
        main_deadline + client.straggler_duration * 1 / 2
    )
    assert recorded[1][1] == pytest.approx(
        main_deadline + client.straggler_duration * 2 / 2
    )
    assert recorded[2][1] == pytest.approx(main_deadline)
    assert recorded[3][1] == pytest.approx(main_deadline)

    recorded = []
    client = _make_client(tmp_path, num_sessions=3, num_stragglers=2)
    monkeypatch.setattr(ec.EvalClient, "eval_session", fake_eval_session)

    async def start2() -> None:
        main_tasks, straggler_tasks, _ = client._start_sessions(1, 1)
        await asyncio.gather(*main_tasks, *straggler_tasks)

    asyncio.run(start2())
    main_deadline = recorded[0][0]
    assert recorded[0][1] == pytest.approx(main_deadline + client.straggler_duration)
    assert recorded[1][1] == pytest.approx(main_deadline + client.straggler_duration)
    assert recorded[2][1] == pytest.approx(main_deadline)
