...
class EvalClient:
    def __init__(
        ...
        self.straggler_taper = straggler_taper
    ...
    def _start_sessions(
        ...
        for i in range(self.num_sessions):
            ...
            if is_straggler and self.straggler_taper:
                # Tapered: straggler i finishes at (i+1)/num_stragglers of the
                # straggler window instead of all at the window's end.
                session_straggler_deadline = (
                    main_deadline
                    + self.straggler_duration * (i + 1) / self.num_stragglers
                )
            elif is_straggler:
            ...
            task = asyncio.create_task(
                self.eval_session(
                    ...
                    session_id=session_id,
                    pinned_version=pinned_version,
                    main_phase_deadline=main_deadline,
...
async def main() -> None:
    ...
    parser.add_argument(
        "--straggler-taper",
        action="store_true",
        help="Stagger straggler end times across the straggler window",
    )
    parser.add_argument(
    ...
    client = EvalClient(
        ...
        straggler_taper=args.straggler_taper,
