...
def deploy_pool_and_spawn(run: Any, *, skip_rollout_ready_check: bool = False) -> Any:
    ...
    run.app.deploy()
    return _await_floor_and_spawn(
        run, skip_rollout_ready_check=skip_rollout_ready_check
    )
...
def _await_floor_and_spawn(run: Any, *, skip_rollout_ready_check: bool) -> Any:
    await_rollout_ready(run.APP_NAME, run.modal_cfg, skip=skip_rollout_ready_check)
