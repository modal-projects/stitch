...
def test_fabricate_delta_dir_produces_publishable_delta(tmp_path: Path) -> None:
    ...
    ref = stitch.publish.publish_version(store, None, str(version_dir), run_id="run")
    ...
    for shard_name in set(index["weight_map"].values()):
        with safe_open(str(version_dir / shard_name), framework="numpy") as f:
            ...
            for name in index["weight_map"]:
                ...
                delta = (
                    zstandard.ZstdDecompressor().decompressobj().decompress(compressed)
                )
...
def test_delta_anchor_dir_resolution(tmp_path: Path) -> None:
    ...
    assert publish_app._delta_anchor_dir(run_dir, 0) == Path(
        publish_app.exp.BASE_CHECKPOINT_PATH
    )
...
def test_fabricate_walk(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """fabricate -> fabricate_delta -> publish: spawn args, 400 guards, and
    reload ordering."""
    from fastapi.testclient import TestClient
    ...
    monkeypatch.setattr(
        publish_app, "fabricate_delta_version", _FakeSpawn("fc-reload-delta")
    )
    server, store = _make_recording_server(
